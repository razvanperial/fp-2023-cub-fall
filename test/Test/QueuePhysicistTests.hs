module Test.QueuePhysicistTests where
...
emptyTest = testGroup "isEmpty"
  [
    ...
    empty (constructor :: QueuePhysicist Int) @?= True

    , testCase "returns False for a non-empty queue" $
    empty (enqueue (constructor :: QueuePhysicist Int) 1) @?= False
...
enqueueTest = testGroup "enqueue"
  [
    ...
    head (enqueue (constructor :: QueuePhysicist Int) 1) @?= 1

    , testCase "adds an element to a non-empty queue" $
    head (enqueue (enqueue (constructor :: QueuePhysicist Int) 1) 2) @?= 1

    , testCase "returns the first element of a non-empty queue" $
    head (enqueue (constructor :: QueuePhysicist Int) 1) @?= 1

    , testCase "removes the first element of a non-empty queue" $
    head (tail (enqueue (enqueue (constructor :: QueuePhysicist Int) 1) 2)) @?= 2
